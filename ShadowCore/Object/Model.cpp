...
void Model::Render() {
    if (Core::isEnableEditor) //  && this->getId() == Core::selected_ObjectID
        RenderComponents();
    if (Model::render_shader != nullptr) {
        ...
        if (Model::material != nullptr && Model::material->material_type == MATERIAL) {
            ...
            Model::render_shader->setValue("material.Ambient", matColor->Ambient);
            Model::render_shader->setValue("material.Diffuse", matColor->Diffuse);
            Model::render_shader->setValue("material.Specular", matColor->Specular);
            Model::render_shader->setValue("material.Emission", matColor->Emission);
            Model::render_shader->setValue("material.Shininess", matColor->Shininess);

            if (matColor->diffuse_texture != nullptr) {
                Model::render_shader->setValue("has_diffuse_texture", 1);
                Model::render_shader->setValue("diffuse_texture", 0);
                matColor->diffuse_texture->Bind(Model::render_shader, 0);
            }
            else {
                Model::render_shader->setValue("has_diffuse_texture", 0);
            }
            if (matColor->specular_texture != nullptr) {
                Model::render_shader->setValue("has_specular_texture", 1);
                Model::render_shader->setValue("specular_texture", 1);
                matColor->specular_texture->Bind(Model::render_shader, 1);
            }
            else {
                Model::render_shader->setValue("has_specular_texture", 0);
            }
            if (matColor->emission_texture != nullptr) {
                Model::render_shader->setValue("has_emission_texture", 1);
                Model::render_shader->setValue("emission_texture", 2);
                matColor->emission_texture->Bind(Model::render_shader, 2);
            }
            else {
                Model::render_shader->setValue("has_emission_texture", 0);
            }
        }
        else {
            Model::render_shader->setValue("material.Ambient", glm::vec3(0.1f));
            Model::render_shader->setValue("material.Diffuse", glm::vec3(1.f));
            Model::render_shader->setValue("material.Specular", glm::vec3(0.5f));
            Model::render_shader->setValue("material.Emission", glm::vec3(0.0f));
            Model::render_shader->setValue("material.Shininess", 32.f);
        ...
        Model::render_shader->setValue("model", Model::transform->GetMatrix());

        if (Core::Engine->level->shadows.size() > 0) {
            Model::render_shader->setValue("lightSpaceMatrix", Core::Engine->level->shadows[0]->GetRenderCam()->GetPVMatrix());
            Model::render_shader->setValue("shadow_map", 5);
            Core::Engine->level->shadows[0]->Bind(5, false);
            Model::render_shader->setValue("has_shadow_map", 1);
        }
