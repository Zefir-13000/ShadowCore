...
void Camera::UpdateProjection(int _width, int _height) {
	Camera::proj = glm::perspective(glm::radians(Camera::FOV) * (float)_width / _height, static_cast<float>(_width) / static_cast<float>(_height), Camera::near_plane, Camera::far_plane);
}
...
{
	if (GetAsyncKeyState(VK_RBUTTON) && firstPress && Camera::isActive) {
		// Fly
		...
		// Look
		//std::cout << "ENABLE_EDITOR: " << ENABLE_EDITOR << std::endl;
	...
	else if (Camera::isActive && GetAsyncKeyState(VK_RBUTTON) && !firstPress) {
		if (Core::isEnableEditor) {
			ImGui::SetMouseCursor(ImGuiMouseCursor_None);
			SetCursorPos(lastMouseX, lastMouseY);
		}
